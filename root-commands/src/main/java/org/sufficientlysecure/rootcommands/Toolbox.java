...
public class Toolbox {
    ...
    public void copyFile(File source, File destination, boolean remountAsRw,
        ...
        try {
            shell.execCommand("toolbox dd if=" + source + " of="
                    + destination);
        } catch (FailedExecuteCommand e) {
            shell.execCommand("toolbox cat " + source + " > "
                    + destination);
        }
     ...
     * @throws IOException
     */
    public void adjustSystemClock(final long offset) throws FailedExecuteCommand,
            TimeoutException, IOException {
        withWritePermissions("/dev/alarm", new WithPermissions() {
    ...
    public boolean remount(File file, String mountType) {
        // Recieved a request, get an instance of Remounter
        Remounter remounter = new Remounter(shell);
        // send the request
        return (remounter.remount(file.getAbsolutePath(), mountType));
    }
